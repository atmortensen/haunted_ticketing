...
class Ticket extends Component {
  ...
  render() {
		if (!this.state.transaction) { return <p></p> }
    ...
    return (
			<Template>
				...
				<FlexBox>
					<QRCode src={'/api/qr/' + this.state.transaction.qr_code} alt="QR Code" />

					<div>
						<p><strong>Name:</strong> { this.state.transaction.customer_name }</p>
						<p><strong>Number of Tickets:</strong> { this.state.transaction.number_of_tickets }</p> 
						<p>
							<strong>Date:</strong> 
							{ ' ' + moment.unix(this.state.transaction.time_slot.start_time).format('dddd, MMM Do') }
						</p>
						<p>
							<strong>Time Slot:</strong>
							{ ' ' + moment.unix(this.state.transaction.time_slot.start_time).format('h:mma') + ' - ' +
							moment.unix(this.state.transaction.time_slot.end_time).format('h:mma') }
						</p>
						<p><strong>Amount Paid:</strong> ${(this.state.transaction.amount_paid / 100).toFixed(2)}</p>
						<p>
